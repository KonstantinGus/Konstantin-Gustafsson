...
# lower_magenta = np.array([170, 150, 150]) vaihtaa tunnistettavien pallojen värin
# upper_magenta = np.array([180, 255, 255])
# lower_green = np.array([28, 150, 200])
# upper_green = np.array([32, 255, 255])


capName ="http://localhost:8080" #tän voi vaihtaa jos kuvafiidi tulee eri paikasta
...
def CapFrame():
    ...
    while True:
        capture = cv2.VideoCapture(capName)
...
#testikäytössä, poistetaan turndegrees ja forfwardmm etc
...
frndPointA =(197,201)
frndPointB = (567,544) #pitää vaihtaa, random pojot joihin frnd palaa cp 0:ssa
...
def FriendIt():
    while True:
        ...
        frspd1 = int(abs(frangle2) / 1.8 * 1.1)
        ...
        if frcheckpoint == 1:
            ...
            if frHelpDist < Distance(frpos1, frtar0): # jos  mentiin ohi lähetetään stop
                frcheckpoint +=1
                SendItFriend(0,0) 
            else:
                SendItFriend(frmoveSpeed, frmoveSpeed)

            frHelpDist2 = Distance(pos1, frtar0)
        ...
        if frcheckpoint == 5:
            frmoveSpeed = Distance(frpos1, frtar1) / 2
            frmoveSpeed = NormalizeSpeed(frmoveSpeed)
            SendItFriend(frmoveSpeed, frmoveSpeed)
            ...
            # elif abs(frangle1) % 90== 0: #jos kulma on suora, ollaan jumissa seinässä
            #     #ei toimi oikeessa elämässä, koska ei koskaan tasan 90 astetta
            #     SendItFriend(-100, -70)
            #     print("seinä")
            #     time.sleep(0.5)
            #     frcheckpoint = 0
...
pointA = (300, 300)  #Keskilinja pitää varmaan hardkoodata, jos ei haluu muuttaa logiikkaa paljoa
pointB = (460, 460)
pointA1 = (500, 100)
pointB1 = (650, 250)
pointA2 = (80, 475)
...
pointOwnGoal = (477, 277) #nää on maalintekoalueiden keskipiste
pointEnemyGoal = (270, 470)
...
while time.time() - start_time < game_time:
    ...
    if checkpoint == 1:
        moveSpeed = Distance(pos1, tar1) / 2 *0.8
